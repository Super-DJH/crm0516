...
<script type="text/javascript">
	...
	$(function(){
		...
		/*关联市场活动的模态窗口 给模糊查询回车绑定事件*/
        $("#aname").keydown(function (event) {

            if(event.keyCode==13){
                //alert($.trim($("#aname").val()))
                ...
                $.ajax({
                    ...
                    success : function (data) {
                        var html = "";
                        //返回{data:[{activityList}{}{}{}]}
						/*n.id是第三张表的id
						* n.name，startDate,endDate是activity表中的数据
						* n.owner是user表中的name*/
		...
		$("#bundBtn").click(function () {
			...
			if ($xz.length==0){
				...
				//通过clueId和activityId和随机生成的uuId为第三张表添加数据
				$.ajax({
					url:"workbench/clue/bund.do?"+a,
					type:"post",
					dataType:"json",
					success : function (data) {
		...
		$("#updateRemarkBtn").click(function () {
			...
			$.ajax({
				...
				dataType:"json",
				success : function (data) {
					//返回{success:true/false,cr:{}}
					if(data.success){
						alert("备注信息修改成功")
						//修改成功后应该局部刷新页面
						//更新div中的信息，
						$("#a"+id).html(data.cr.noteContent);
			...
			$.ajax({
				...
				success : function (data) {
					...
					if(data.success){
						...
						html += '<h5 id="a'+data.cr.id+'">'+data.cr.noteContent+'</h5>';
	...
	function showClueRemarkList() {
		$.ajax({
			url:"workbench/clue/showClueRemarkListByClueId.do",
			data:{
				"clueId":"${c.id}"
			},
			type:"get",
