...
<script type="text/javascript">
            ...
            $.ajax({
                ...
                success : function (data) {
                    ...
                    if(data.success){
						...
						html += '<h5 id="a'+data.ar.id+'">'+data.ar.noteContent+'</h5>',
	...
	function showRemarkList() {
		...
		$.ajax({
			url:"workbench/activity/showRemarkListByAid.do",
			data:{
				"activityId":"${a.id}",
			},
			type:"get",
			...
			success : function (data) {
				...
				$.each(data,function (i,t) {
					html += '<div id="'+t.id+'" class="remarkDiv" style="height: 60px;">',
					html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">',
					html += '<div style="position: relative; top: -40px; left: 40px;" >',
					html += '<h5 id="a'+t.id+'">'+t.noteContent+'</h5>',
					html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${a.name}</b> <small style="color: gray;" id="s'+t.id+'" > '+(t.editFlag==0?t.createTime:t.editTime)+' 由'+(t.editFlag==0?t.createBy:t.editBy)+'</small>',
					html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">',
					html += '<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+t.id+'\')" ><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #FF0000;"></span></a>',
					html += '&nbsp;&nbsp;&nbsp;&nbsp;',
					html += '<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\''+t.id+'\')" ><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #FF0000;"></span></a>',
					html += '</div>',
					html += '</div>',
					html += '</div>'

				})
                /*动态生成的onclick里加参数应该是先转义 \'\' （必须得套在双引号中，将单引号转义为双引号） 再在里面拼加属性*/
				$("#remarkDiv").before(html)
			}
        ...
        $.ajax({
            ...
            success : function (data) {
                ...
                if(data.success){
                    alert("删除成功")
                    //删除成功后，重新展示备注信息列表
                    //showRemarkList();  此方法不行，会给页面再次展示方法中查询到的，然后追加在前面
                    //给div动态的加id，通过id将div删除
                    $("#"+id).remove()
                }else{
                    alert("删除失败")
                }
